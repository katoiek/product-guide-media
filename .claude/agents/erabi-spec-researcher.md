...
- **`name` は officialUrl のページに表示されている製品名をそのまま書く。** 通称・シリーズ名・記憶している名前で書かない。記録する前に、そのページの製品名と一致していることを必ず確認する。
- **別ページの情報を混ぜるときは、必ず出典を値の中に書く。** 製品ページ本文に無く、同じメーカーのQ&Aや成分情報ページから取った項目は「製品ページ本文には記載なし。〈URL〉に記載」の形にする。どのページから来た値かが後から分からなくなる書き方をしない。
- **officialUrl のページに載っていない容量・バリエーションを `specs` に足さない。** 「本体1.1L／リフィル4.0L」のようにページ外の情報を混ぜない。
